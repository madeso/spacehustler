...
bool ImGui_ImplSdlGL3_ProcessEvent(SDL_Event* event)
{
    ...
    case SDL_MOUSEBUTTONUP:
        {
            if (event->button.button == SDL_BUTTON_LEFT) io.MouseDown[0] = g_MousePressed[0] = event->type == SDL_MOUSEBUTTONDOWN;
            if (event->button.button == SDL_BUTTON_RIGHT) io.MouseDown[1] = g_MousePressed[1] = event->type == SDL_MOUSEBUTTONDOWN;
            if (event->button.button == SDL_BUTTON_MIDDLE) io.MouseDown[2] = g_MousePressed[2] = event->type == SDL_MOUSEBUTTONDOWN;
            return true;
        }
    case SDL_TEXTINPUT:
        {
...
void ImGui_ImplSdlGL3_NewFrame(int w, int h, int display_w, int display_h, int mx, int my, bool mouseLeft, bool mouseRight, bool mouseMiddle)
{
    if (!g_FontTexture)
