...
KeyDef::KeyDef(const Json::Value& data, const InputActionMap& map) {
  for (Json::ArrayIndex i = 0; i < data.size(); ++i) {
    ...
    if (type == "button") {
      ...
      const KeyType key = Converter::FromString(keyname);
      ...
      binds_.push_back(Bind<KeyType>(key, action));
    } else if (type == "axiskeys") {
      const std::string minname = d.get("min", "").asString();
      const KeyType minkey = Converter::FromString(minname);
      ...
      const KeyType maxkey = Converter::FromString(maxname);
      ...
      axiskeys_.push_back(AxisKeyBind<KeyType>(minkey, maxkey, action, d));
    } else {
