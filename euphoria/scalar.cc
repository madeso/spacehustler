...
const float Curve(const float newValue, const float oldValue,
                  const float smoothingValue) {
  const int sign = SignOf(oldValue - newValue);
  const float slip = (oldValue - newValue) / smoothingValue;
  const float val = oldValue - slip;
  if (sign != SignOf(val - newValue)) {
    return newValue;
  } else {
    return val;
  }
...
const float Square(const float r) { return r * r; }
...
float UpperBound(float num, float gran) { return std::ceil(num / gran) * gran; }
}  // namespace
