...
boost::shared_ptr<Shader> Shader::FromSource(const std::string& source, const Type& type)
{
...
boost::shared_ptr<Program> Program::FromShaderList(const ShaderList& shaders)
{
	struct Local
	{
		static void AttachShader(GLuint prog, const boost::shared_ptr<Shader>& shader)
		{
			glAttachShader(prog, shader->get());
		}
		static void DetachShader(GLuint prog, const boost::shared_ptr<Shader>& shader)
		{
	...
	std::for_each(shaders.shaders.begin(), shaders.shaders.end(), boost::bind(&Local::AttachShader, program->get(), _1));
	glLinkProgram(program->get());
	std::for_each(shaders.shaders.begin(), shaders.shaders.end(), boost::bind(&Local::DetachShader, program->get(), _1));
